...
后续随着工具建设的完善以及自测比例的增加，RD的参与度会越来越高，所以优先考虑对核心链路信息的维护和更新。当前已注解的形式进行信息收集更为妥当。
当然如果服务较为稳定，核心链路的变更较少，那么可以采用文本的方式记录，具体方式可由各业务线主R根据自身业务特点进行灵活选择。 
...
服务引入jar包，并添加链路注解后，需要添加rattler配置文件，示例如下：
```text
...
rattler-func-detetion 是一个基于Git Patch文件的开源代码分析工具，主要用于函数变动探测。通过以下命令：
```bash
