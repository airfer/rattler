...
import static com.airfer.rattler.sandbox.watcher.EventWatchBuilder.PatternType.WILDCARD;
...
/**
 * <p>为何兼容原有的sandbox api方法, 本构造者在此基础上进行扩充。由于原该类的方法是私有封装，没有办法通过extends来进行修改
 * 所以只能以此为蓝本来构建新的methods
 *
 * <p>该构建者将部分原有私有的class和method的访问属性变更为public,后续可以此通过继承自定义EventWatchBuilder，在次再次感谢
 * sandbox原有团队所做的努力~
 *
 * 事件观察者类构建器
 * <p>
...
public class EventWatchBuilder {
    ...
    public class BuildingForBehavior implements IBuildingForBehavior {
        ...
        @Override
        public IBuildingForBehavior onStrategy(InjectionStrategy strategy) {
